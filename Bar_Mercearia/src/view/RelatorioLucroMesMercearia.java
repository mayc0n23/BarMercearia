...
public class RelatorioLucroMesMercearia implements StrategyRelatorio{
	...
	public void gerarRelatorio(String mes, int id) {
		...
		try {
			float total = 0;
			PdfWriter.getInstance(doc, new FileOutputStream("lucro_mes_mercearia.pdf"));
			doc.open();
			Paragraph p = new Paragraph("Lucro do mês de " + mes + " da Mercearia");
			...
			try {
				desk.open(new File("lucro_mes_mercearia.pdf"));
			}catch(Exception e) {}
